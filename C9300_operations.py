...
configured_password = credentials.login['aaa_password']
configured_secret = credentials.login['aaa_secret']
access_ports = b'int range g1/0/1-22'
trunk_ports_copper = b'int range g1/0/22-24'
trunk_ports_fiber = b'int range g1/1/1-4'
src_int = b'vlan 10'
...
def login(console):
    ...
    while True:
        ...
        if b'[yes/no]:' in input_data:
            console.write(b'no')
        elif b'Authentication' in input_data:
            print(send_command(console, cmd=current_password))
            time.sleep(.5)
        elif b'>' in input_data:
            console.write(b'en')
        elif b'Password' in input_data:
            print(send_command(console, cmd=current_secret))
            time.sleep(.5)
...
def global_commands(console):
    ...
    print(send_command(console, cmd=b'shell proc full'))
...
def trunk_int(console):
    print(send_command(console, cmd=trunk_ports_copper))
    print(send_command(console, cmd=b'sw m t'))
    print(send_command(console, cmd=b'ip dh sn t'))
    print(send_command(console, cmd=b'exit'))
    print(send_command(console, cmd=trunk_ports_fiber))
    print(send_command(console, cmd=b'sw m t'))
    print(send_command(console, cmd=b'ip dh sn t'))
    print(send_command(console, cmd=b'exit'))
...
def tacacs_commands(console):
    print(send_command(console, cmd=b'tacacs server nskisetacpsn'))
    print(send_command(console, cmd=b'address ipv4 10.26.60.20'))
    print(send_command(console, cmd=b'key 7 047D02422724554B5B'))
    print(send_command(console, cmd=b'tacacs server bvlisetacpsn'))
    print(send_command(console, cmd=b'address ipv4 158.139.170.16'))
    print(send_command(console, cmd=b'key 7 047D02422724554B5B'))
    print(send_command(console, cmd=b'tacacs server cq9isetacpsn'))
    print(send_command(console, cmd=b'address ipv4 155.191.182.137'))
    print(send_command(console, cmd=b'key 7 047D02422724554B5B'))
